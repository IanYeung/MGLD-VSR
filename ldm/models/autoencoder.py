...
from ldm.modules.diffusionmodules.util import from_5d_to_4d, from_4d_to_5d
from ldm.modules.diffusionmodules.model import Encoder, Decoder, Decoder_Mix, VideoDecoder, VideoDecoder_Mix, VideoDecoderV2, VideoDecoder_MixV2
...
class VideoAutoencoderKL_L2O(pl.LightningModule):
    def __init__(self,
                 ddconfig,
                 lossconfig,
                 embed_dim,
...
class VideoAutoencoderKLResi(pl.LightningModule):
    def __init__(self,
                 ddconfig,
                 lossconfig,
                 embed_dim,
        ...
        self.encoder = Encoder(**ddconfig)
        if version == 1:
            self.decoder = VideoDecoder_Mix(**ddconfig)
        else:
            self.decoder = VideoDecoder_MixV2(**ddconfig)
        self.decoder.fusion_w = fusion_w
        self.loss = instantiate_from_config(lossconfig)
        self.quant_conv = torch.nn.Conv2d(2*ddconfig["z_channels"], 2*embed_dim, 1)
        self.post_quant_conv = torch.nn.Conv2d(embed_dim, ddconfig["z_channels"], 1)
