...
from ldm.modules.diffusionmodules.util import from_4d_to_3d, from_3d_to_4d
...
class MemoryEfficientSelfAttention(nn.Module):
    ...
    def __init__(self, query_dim, heads=8, dim_head=64, dropout=0.0):
        super().__init__()
        print(f"Setting up {self.__class__.__name__}. Query dim is {query_dim}, using {heads} heads.")
        inner_dim = dim_head * heads
...
class MemoryEfficientCrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if x.shape[0] != context.shape[0]:
            context = torch.repeat_interleave(context, x.shape[0] // context.shape[0], dim=0)
        k = self.to_k(context)
        v = self.to_v(context)
        ...
        #     .contiguous()
        # k = k.unsqueeze(3)\
        #     .reshape(b, k.shape[1], self.heads, self.dim_head)\
        #     .permute(0, 2, 1, 3)\
        #     .reshape(b * self.heads, k.shape[1], self.dim_head)\
        #     .contiguous()
        # v = v.unsqueeze(3)\
        #     .reshape(b, v.shape[1], self.heads, self.dim_head)\
        #     .permute(0, 2, 1, 3)\
        #     .reshape(b * self.heads, v.shape[1], self.dim_head)\
        #     .contiguous()
