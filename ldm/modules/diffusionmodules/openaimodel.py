...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb):
        ...
        #     h = torch.repeat_interleave(h, emb_out.shape[0] // h.shape[0], dim=0)
...
class InflatedUNetModel(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
...
class InflatedUNetModelDualcondV2(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            for nr in range(self.num_res_blocks[level]):
                layers = [
                    ...
                    # # temporal mixing
                    # SpatialTemporalConv(mult * model_channels, num_frames=num_frames)
                ]
                ch = mult * model_channels
                if ds in attention_resolutions:
                    if num_head_channels == -1:
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        ...
                        # # temporal attn
                        # layers.append(
                        #     TemporalAttention(ch, num_heads=num_heads, dim_head=dim_head, num_frames=num_frames)
                        # )
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
                input_block_chans.append(ch)
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            SpatialTransformerV2(  # always uses a self-attn
                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
                disable_self_attn=disable_middle_self_attn, use_linear=use_linear_in_transformer,
                use_checkpoint=use_checkpoint
            ),
...
class InflatedEncoderUNetModelWT(nn.Module):
    """
    The half UNet model with attention and timestep embedding.
    For usage, see UNet.
    """
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    # # temporal attn
                    # layers.append(
                    #     TemporalAttention(ch, num_heads=num_heads, dim_head=ch // num_heads, num_frames=num_frames)
                    # )
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
            if level != len(channel_mult) - 1:
...
class VideoEncoderUNetModelWT(nn.Module):
    """
    The half UNet model with attention and timestep embedding.
    For usage, see UNet.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.propagation = CouplePropModule(num_ch=in_channels, num_feat=64, num_block=2)
    ...
    def forward(self, x, timesteps, flows=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        x = rearrange(x, "(b n) c h w -> b n c h w", n=self.num_frames)
        x = self.propagation(x, flows)
