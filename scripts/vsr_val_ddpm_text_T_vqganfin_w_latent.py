...
def main():
    ...
    # Model
    vqgan_config = OmegaConf.load("configs/video_vae/video_autoencoder_kl_64x64x4_resi.yaml")
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for n in trange(len(init_segment_list), desc="Sampling"):
                        ...
                        os.makedirs(os.path.join(opt.latent_dir, seq_item), exist_ok=True)
                        for i in range(init_image.size(0)):
                            ...
                            with open(os.path.join(opt.latent_dir, seq_item, basename + '.npy'), 'wb') as f:
                                np.save(f, samples[i].cpu().numpy())
                    ender.record()
                    toc = time.time()
                    # WAIT FOR GPU SYNC
